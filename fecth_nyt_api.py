...
abstract = map(lambda x: x.get("abstract", ""), articles)
web_url = map(lambda x: x.get("web_url", ""), articles)
snippet = map(lambda x: x.get("snippet", ""), articles)
lead_paragraph = map(lambda x: x.get("lead_paragraph", ""), articles)
source = map(lambda x: x.get("source", ""), articles)
pub_date = map(lambda x: x.get("pub_date", ""), articles)
document_type = map(lambda x: x.get("document_type", ""), articles)
news_desk = map(lambda x: x.get("news_desk", ""), articles)
section_name = map(lambda x: x.get("section_name", ""), articles)
type_of_material = map(lambda x: x.get("type_of_material", ""), articles)
word_count = map(lambda x: x.get("word_count", ""), articles)
article_id = map(lambda x: x.get("_id", ""), articles)
...
#save new data
pq.write_table(new_data, f'/home/ahyar/final_project/data_lake/NYT_{date}.parquet')

#load existing table
existing_data = pq.read_table('/home/ahyar/final_project/artificial-intelligence_Jan-Mei_copy.parquet')

#print how many existing article
existing_article = len(existing_data)
...
#print total article after added
new_existing_data = pq.read_table('/home/ahyar/final_project/artificial-intelligence_Jan-Mei_copy.parquet')
total_article = len(new_existing_data)
print(f'Total article: {total_article}')

#check succed or not adding new article
total = new_article + existing_article
if total_article == total:
    print('Succeed to adding new article')
else:
    print('Failed to adding new article')